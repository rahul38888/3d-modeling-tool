...
inline void readstr(FILE *f, char *string)
{
    do {
    fgets(string, 255, f);
...
struct object * file_loader(const float * num, struct object *last, struct camera *cam, struct light *light, int type)
{
    if(type==1){
        ...
        if(last->type==1 || last->type==2){
            ...
            last->mat = static_cast<struct materialType *>(malloc(sizeof(struct materialType)));
        ...
        light->number = (int)num[1];

        light->light_ambient[0] = (GLfloat)num[2];
        light->light_ambient[1] = (GLfloat)num[3];
        light->light_ambient[2] = (GLfloat)num[4];

        light->light_diffuse[0] = (GLfloat)num[5];
        light->light_diffuse[1] = (GLfloat)num[6];
        light->light_diffuse[2] = (GLfloat)num[7];

        light->light_specular[0] = (GLfloat)num[8];
        light->light_specular[1] = (GLfloat)num[9];
        light->light_specular[2] = (GLfloat)num[10];

        light->light_position[0] = (GLfloat)num[11];
        light->light_position[1] = (GLfloat)num[12];
        light->light_position[2] = (GLfloat)num[13];

        light->light_attenuation[0] = (GLfloat)num[14];
        light->light_attenuation[1] = (GLfloat)num[15];
