...
inline void draw_light(struct light *light[], GLenum globalSmooth){
        ...
        for(int i=0;i<8;i++){
            ...
            glLightfv(return_light(light[i]->number), GL_SPECULAR, light_specular);
            glLightfv(return_light(light[i]->number),GL_POSITION, light_position);
            if(light[i]->number){
                if(light[i]->light_attenuation[0]){
                        glLightf( return_light(light[i]->number),GL_LINEAR_ATTENUATION , light_attenuation);
                }
            }

            if(light[i]->on){
                if(light[i]->number) glDisable(GL_LIGHT0);

                glEnable(return_light(light[i]->number));
                if(light[i]->number){
                    glDisable(GL_LIGHTING);
                    glPushMatrix();
                    glColor3f(1,1,0);
                        glTranslated(light_position[0],light_position[1],light_position[2]);
                        glutSolidCube(0.2*scale_factor_local);
                    glPopMatrix();
                    glEnable(GL_LIGHTING);
                }
                /*else{
                    ...
                    glColor3f(1,1,0);
                        glTranslated(light_position[0],light_position[1],light_position[2]);
                        glutSolidSphere(0.1*scale_factor_local,8,8);
                    glPopMatrix();
                    glEnable(GL_LIGHTING);
                }*/
            }
            else{
                glDisable(return_light(light[i]->number));
            }
        }
